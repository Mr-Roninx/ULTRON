...
class EconomicVirtualClock:
    """
    Authoritative deterministic virtual clock for the persistent economic civilization.
    """
    ...
    def advance_to(self, target_timestamp: int):
        if target_timestamp < self._current_time:
            raise ValueError(f"Cannot advance clock backwards: {target_timestamp} < {self._current_time}")
    ...
    def reset(self, new_timestamp: Optional[int] = None):
        t = new_timestamp if new_timestamp is not None else self._start_time
        self._current_time = t
        self._start_time = t

economic_clock = EconomicVirtualClock()
